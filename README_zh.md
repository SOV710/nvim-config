...
**你已经在维护自己的配置了。** 你在试图组织 20+ 种语言的支持而不让整件事变成一团乱麻，你已经不想再从别人的 `init.lua` 复制粘贴，或者你想看看一份 from-scratch 的架构在有一定规模时长什么样。这里的东西随便拿走。

**或者：你正在用 LazyVim、NvChad、AstroVim、LunarVim**，并且开始感觉到它的边界。你说不出具体想改什么，但你知道你的编辑器在做一些你不完全理解的事情。这份 repo 是一个从零开始的替代方案——有主见、有结构、小到可以完整读完。
